...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  name_field = document.getElementById('enchant_name');
  label_field = document.getElementById('enchant_label');
  email_field = document.getElementById('enchant_email');
  subject_field = document.getElementById('enchant_subject');
  message_field = document.getElementById('enchant_message');
  mailbox_field = document.getElementById('mailbox');
  submit_button = document.getElementById('enchant_submit');
  files = document.querySelectorAll('[id^=files]');
  e_fname = document.getElementById('enchant_fname');
  ...
  if(whoaValidation() == true) {
    ...
    switch(true) {
      case /modeling/.test(_subject):
        ...
        dataset = {name: m[0].trim(), email: email_field.value.trim(), subject: subject_field.value.trim(), message: m[1].trim(), attachments: attachments_set};
        break;
      default:
        dataset = {name: name_field.value.trim(), email: email_field.value.trim(), subject: subject_field.value.trim(), message: message_field.value.trim(), attachments: attachments_set};
    }
...
// Setup XHR
function setXhr() {
  req.open("POST", "https://api1.whoamapi.com/tickets/intake");
...
function whoaValidation() {
  ...
  form_list.forEach(function(f,i) {
    ...
    required_elements.forEach(function(el,i){
      ...
      if(el.value == "") {
        throwMessage(el.name.replace('enchant_','')+" is required.", el, true);
        // Give .whoa-error className
        if(!el.classList.contains('whoa-error')) {
            el.className += " whoa-error";
        }
        errors.push(el);
      }
      else{
      	if(el.id == "enchant_email")
        {
          var re = /^(([^<>()[\]{}'^?\\.,!|\/\/#%*-+=&;:\s@\"]+(\.[^<>()[\]\\.,;:\s@\"]+)*)|(\".+\"))@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/;
  ...
  // Set Listeners
  document.querySelectorAll('.whoa-error').forEach(function(error,i) {
    error.addEventListener('focus', function(e) {
      this.classList.remove('whoa-error');
